import fs from 'fs';
console.log('SCRIPT VERSION: v4 (dates from final revs)');
...
for(let r=1;r<rows.length;r++){
  ...
  for(const {cost,date} of dayMap.values()){ if(prev!==null&&cost===prev)continue; revs.push({cost,date}); prev=cost;}
  // даты берём из ИТОГОВЫХ ревизий (после схлопывания повторов)
  const rowOldest=revs.length?revs[0].date:null;
  const rowLatest=revs.length?revs[revs.length-1].date:null;
  if(dbg<5){dbg++; console.log('DBG',offer,'| oldest:',rowOldest&&rowOldest.toLocaleDateString('ru-RU'),'| latest:',rowLatest&&rowLatest.toLocaleDateString('ru-RU'),'| revs:',revs.length);}
  ...
  if(revs.length)await fetch(process.env.SUPABASE_URL+'/rest/v1/product_prices',{method:'POST',headers:{...HD,Prefer:'return=minimal'},body:JSON.stringify(revs.map(x=>({product_id:pid,cost:x.cost,created_by:null,created_at:x.date.toISOString()})))});
